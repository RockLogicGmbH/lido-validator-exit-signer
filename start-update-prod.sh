#!/bin/bash
# -----------------------------------------------------------------------------
# What: Update, build and run production stack via manual build/start scripts
# Note: The "-d" flag is enforced in any case to provide a smooth update.
# Usage: bash start-update.sh [<composearg>, ..] [<service>, ..]
# Examples:
# bash start-update.sh
# bash start-update.sh -timeout 5
# bash start-update.sh -timeout 5 web 
# -----------------------------------------------------------------------------
...
git fetch || die "error: could not fetch changes from remote repository"

# Compare the local branch with the remote branch and do the update
LOCAL=$(git rev-parse @)
...
if [ $LOCAL = $REMOTE ]; then
	...
	# Stop stack
    say "Stop stack..."
	sudo docker compose -f docker-compose.yaml down "$@" || die "error: could not stop docker container(s)"

	# Pull updates
    say "Pull updates..."
	git pull || die "error: could not pull latest updates"

	# Remove untracked files and directiries
	# This does not remove files in .gitignore like .env
    say "Clean application directory..."
	git clean -fd || die "error: could not clean application directory"

	# Build production containers
    say "Build production containers..."
	bash start-build-prod.sh "$@" || exit $?

	# Start production containers
    say "Start production containers..."
	bash start-prod.sh -d "$@" || exit $?

	# Done
	say "Stack successfully updated"

elif [ $REMOTE = $BASE ]; then
    die "error: local changes detected that are not pushed, avoid update."
else
    die "error: the local and remote branches have diverged"
fi