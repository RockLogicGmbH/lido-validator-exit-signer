#!/bin/bash
# -----------------------------------------------------------------------------
# What: Start manually built production images via docker compose
# Usage: bash start-prod.sh [<composearg>, ..] [<service>, ..]
# Examples:
# bash start-prod.sh
# bash start-prod.sh -d 
# bash start-prod.sh -d web 
# -----------------------------------------------------------------------------
...
sudo docker compose -f docker-compose.yaml up "$@" || die "error: could not start docker container(s)"

# Done
say "Production containers successfully started"