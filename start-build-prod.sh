#!/bin/bash
# -----------------------------------------------------------------------------
# What: Build production iamges manually via docker compose
# Usage: bash start-build-prod.sh [<composearg>, ..] [<service>, ..]
# Examples:
# bash start-build-prod.sh
# bash start-build-prod.sh --no-cache 
# bash start-build-prod.sh --no-cache web 
# -----------------------------------------------------------------------------
...
build "$@" || die "error: build for production failed"

# Done
say "Build successful"
say "You may run production container via:"
say "bash start-prod.sh"
say "Or alternatively via:"
say "docker compose -f docker-compose.yaml"
say "Do not forget to add the '-f docker-compose.yaml' or you need"
say "to delete the 'docker-compose.override.yaml' file before start!"
